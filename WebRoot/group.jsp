...
<div class="fengsu"></div>
			...
			页	</div>  <div class="tgfy">
 					<a href="showlist.action?page.pageIndex=1">首页</a> 
					<a <s:if test="page.pageIndex<=1"> href="javascript:false;"</s:if>
					<s:else>href="showlist.action?page.pageIndex=<s:property value='page.pageIndex-1' />"</s:else>>上一页</a> 
					<a <s:if test="page.pageIndex>=page.totalpages" > href="javascript:false;"</s:if>
					<s:else>href="showlist.action?page.pageIndex=<s:property value='page.pageIndex+1' />"</s:else>>下一页</a> 
					<a href="showlist.action?page.pageIndex=<s:property value="page.totalpages" />">尾页</a></div>
<!--主体内容第一部分结束-->
<!--底部-->
