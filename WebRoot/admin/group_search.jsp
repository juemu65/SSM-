...
<div class="fengsu"></div>
     ...
     <div class="tgfy">当前显示第
			...
			页
                    ...
                    <a href="search.action?page.pageIndex=1">首页</a> 
                     <a <s:if test="page.pageIndex<=1"> href="javascript:false;"</s:if>
					<s:else>href="search.action?page.pageIndex=<s:property value='page.pageIndex-1' />"</s:else>>上一页</a> 
					<a <s:if test="page.pageIndex>=page.totalpages" > href="javascript:false;"</s:if>
					<s:else>href="search.action?page.pageIndex=<s:property value='page.pageIndex+1' />"</s:else>>下一页</a> 
					<a href="search.action?page.pageIndex=<s:property value="page.totalpages" />">尾页</a></div>
<!--主体内容第一部分结束-->
<!--底部-->
